...
[MCP](https://modelcontextprotocol.io) server exposing the EnergyCharts SDK as
two agent tools — `energy-charts_list` and `energy-charts_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/energy-charts-mcp)
make build

# 2. Provide credentials via the environment
export ENERGY_CHARTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user energy-charts \
  -- /absolute/path/to/energy-charts-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./energy-charts-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// energy-charts_list: first page of records
{ "entity": "daily_avg_dict" }
{ "entity": "daily_avg_dict", "query": { } }

// energy-charts_load: one record by id
{ "entity": "cross_border_model", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/energy-charts-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user energy-charts \
     -- "$PWD"/dist/*/energy-charts-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `energy-charts_list` and `energy-charts_load` tools now appear
   in new sessions. Ask the agent to *"list daily_avg_dict using energy-charts"*
   and it calls `energy-charts_list` with `{"entity":"daily_avg_dict"}`.
...
```sh
export ENERGY_CHARTS_APIKEY=sk_live_xxx            # API key
export ENERGY_CHARTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `energy-charts_list` tool
...
### Call the `energy-charts_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `energy-charts_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `energy-charts_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ENERGY_CHARTS_APIKEY` | API key sent with every request. |
| `ENERGY_CHARTS_BASE` | Optional override of the API base URL. |
...
```sh
./energy-charts-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"energy-charts_load","arguments":{"entity":"cross_border_model","query":{"id":1}}}}'
```
