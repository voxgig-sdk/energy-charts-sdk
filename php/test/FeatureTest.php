...
class FtCtrl extends EnergyChartsControl
{
...
class FtClient
{
    ...
    private EnergyChartsUtility $utility;

    public function __construct(EnergyChartsUtility $utility, array $options)
    {
    ...
    public function get_utility(): EnergyChartsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EnergyChartsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EnergyChartsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EnergyChartsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EnergyChartsConfig::make_feature($name);
    ...
    private static function buildUrl(EnergyChartsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EnergyChartsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EnergyChartsResponse($fetched);
        }
    ...
    private function fail(EnergyChartsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EnergyChartsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
