...
class CrossBorderModelEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EnergyChartsSDK::test(null, null);
        $ent = $testsdk->CrossBorderModel(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = cross_border_model_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cross_border_model." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ENERGY_CHARTS_TEST_CROSS_BORDER_MODEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $cross_border_model_ref01_data = null;
        if (count($cross_border_model_ref01_data_raw) > 0) {
...
function cross_border_model_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cross_border_model/CrossBorderModelTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cross_border_model01", "cross_border_model02", "cross_border_model03"] as $k) {
    ...
    $entid_env_raw = getenv("ENERGY_CHARTS_TEST_CROSS_BORDER_MODEL_ENTID");
