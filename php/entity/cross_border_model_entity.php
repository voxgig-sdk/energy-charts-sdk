...
// EnergyCharts SDK CrossBorderModel entity
...
class CrossBorderModelEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CrossBorderModelEntity($this->_client, $opts);
    }

    /**
     * @param CrossBorderModel|array $args CrossBorderModel data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EnergyChartsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CrossBorderModel|array The current CrossBorderModel data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CrossBorderModel fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EnergyChartsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CrossBorderModel fields).
     */
    ...
    /**
     * Load a single CrossBorderModel.
     *
     * @param CrossBorderModelLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CrossBorderModelLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CrossBorderModel|array The loaded CrossBorderModel as an assoc-array at the
     *   SDK boundary; throws EnergyChartsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EnergyChartsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
