...
class CrossBorderModelEntity
  ...
  def make
    opts = @_entopts.dup
    CrossBorderModelEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EnergyChartsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CrossBorderModel, Hash] the current CrossBorderModel data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EnergyChartsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CrossBorderModel fields)
  def match_get
  ...
  #
  # @param reqmatch [CrossBorderModelLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CrossBorderModel.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CrossBorderModel, Hash] the loaded CrossBorderModel; raises EnergyChartsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EnergyChartsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
