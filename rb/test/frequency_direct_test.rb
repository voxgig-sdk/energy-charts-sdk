...
class FrequencyDirectTest < Minitest::Test
  def test_direct_list_frequency
    setup = frequency_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-frequency", setup[:live] ? "live" : "unit")
    if _should_skip
...
def frequency_direct_setup(mockres)
