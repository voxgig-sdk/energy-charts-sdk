...
class RenShareModelDirectTest < Minitest::Test
  def test_direct_list_ren_share_model
    setup = ren_share_model_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-ren_share_model", setup[:live] ? "live" : "unit")
    if _should_skip
...
def ren_share_model_direct_setup(mockres)
