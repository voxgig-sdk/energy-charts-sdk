...
class DailyAvgDictEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DailyAvgDict(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DailyAvgDict(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = daily_avg_dict_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "daily_avg_dict." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ENERGY_CHARTS_TEST_DAILY_AVG_DICT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    daily_avg_dict_ref01_data = nil
    if daily_avg_dict_ref01_data_raw.length > 0
...
def daily_avg_dict_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "daily_avg_dict", "DailyAvgDictTestData.json")
  ...
  entid_env_raw = ENV["ENERGY_CHARTS_TEST_DAILY_AVG_DICT_ENTID"]
