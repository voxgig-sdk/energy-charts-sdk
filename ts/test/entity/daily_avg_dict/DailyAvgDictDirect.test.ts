...
describe('DailyAvgDictDirect', async () => {
  ...
  test('direct-list-daily_avg_dict', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-daily_avg_dict', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ENERGY_CHARTS_TEST_DAILY_AVG_DICT_ENTID']
