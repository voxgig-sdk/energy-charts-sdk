...
describe('RenShareModelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'ren_share_model.' + op, live)) return
    }
    ...
    const ren_share_model_ref01_list = await ren_share_model_ref01_ent.list(ren_share_model_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ENERGY_CHARTS_TEST_REN_SHARE_MODEL_ENTID']
