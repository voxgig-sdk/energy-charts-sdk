...
describe('CrossBorderModelDirect', async () => {
  ...
  test('direct-load-cross_border_model', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-cross_border_model', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ENERGY_CHARTS_TEST_CROSS_BORDER_MODEL_ENTID']
