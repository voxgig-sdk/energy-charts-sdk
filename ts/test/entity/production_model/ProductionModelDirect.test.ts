...
describe('ProductionModelDirect', async () => {
  ...
  test('direct-load-production_model', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-production_model', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ENERGY_CHARTS_TEST_PRODUCTION_MODEL_ENTID']
