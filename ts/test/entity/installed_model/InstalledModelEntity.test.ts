...
describe('InstalledModelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'installed_model.' + op, live)) return
    }
    ...
    const installed_model_ref01_list = await installed_model_ref01_ent.list(installed_model_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ENERGY_CHARTS_TEST_INSTALLED_MODEL_ENTID']
  ...
  if (live) {
    client = new EnergyChartsSDK(merge([
      {
        apikey: env.ENERGY_CHARTS_APIKEY,
      },
      extra
    ]))
  }
