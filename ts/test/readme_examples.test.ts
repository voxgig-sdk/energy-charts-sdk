...
//     (\`new EnergyChartsSDK(...)\` / \`EnergyChartsSDK.test(...)\` become
//     \`EnergyChartsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EnergyChartsSDK'
...
const TEST_SEED = {"entity":{"cross_border_model":{"test01":{"id":"test01"}},"daily_avg_dict":{"test01":{"id":"test01"}},"frequency":{"test01":{"id":"test01"}},"installed_model":{"test01":{"id":"test01"}},"price":{"test01":{"id":"test01"}},"production_model":{"test01":{"id":"test01"}},"public_power_forecast":{"test01":{"id":"test01"}},"ren_share_model":{"test01":{"id":"test01"}},"share_model":{"test01":{"id":"test01"}},"traffic_model":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EnergyChartsSDK(...)\` and any
// \`EnergyChartsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EnergyChartsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EnergyChartsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
