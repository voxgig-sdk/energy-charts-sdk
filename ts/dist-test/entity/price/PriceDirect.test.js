...
(0, node_test_1.describe)('PriceDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-price', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-price', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['ENERGY_CHARTS_TEST_PRICE_ENTID'];
