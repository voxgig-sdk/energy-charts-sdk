...
module EnergyChartsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EnergyChartsResponse.new({})
    ...
    if fetch_err
      ...
      response = EnergyChartsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EnergyChartsResponse.new(fetched)
    else
