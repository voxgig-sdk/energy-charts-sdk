...
class EnergyChartsMakeError
{
    public static function call(?EnergyChartsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EnergyChartsContext([], null);
        }
        $op = $ctx->op ?? new EnergyChartsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EnergyChartsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EnergyChartsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EnergyChartsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof EnergyChartsError) {
