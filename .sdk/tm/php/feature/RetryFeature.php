...
// EnergyCharts SDK retry feature
...
class EnergyChartsRetryFeature extends EnergyChartsBaseFeature
{
    ...
    public function init(EnergyChartsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EnergyChartsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EnergyChartsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EnergyChartsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
