// Command EnergyCharts-cli is a CLI + REPL for the EnergyCharts SDK,
...
const prompt = "energy-charts"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cross_border_model daily_avg_dict frequency installed_model price production_model public_power_forecast ren_share_model share_model traffic_model"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ENERGY_CHARTS_APIKEY (api key), ENERGY_CHARTS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ENERGY_CHARTS_APIKEY carries the API key and
	// ENERGY_CHARTS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ENERGY_CHARTS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ENERGY_CHARTS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEnergyChartsSDK(opts)
