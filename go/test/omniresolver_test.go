...
import (
	...
	omni "github.com/voxgig-sdk/energy-charts-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EnergyChartsError); is {
				out := map[string]any{
					"name":    "EnergyChartsError",
...
func omniCtx(arg any, client *sdk.EnergyChartsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EnergyChartsSDK); is {
			client = live
