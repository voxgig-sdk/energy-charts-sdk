...
func TestProductionModelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		productionModelRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.production_model")))
...
func production_modelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ENERGY_CHARTS_TEST_PRODUCTION_MODEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ENERGY_CHARTS_TEST_PRODUCTION_MODEL_ENTID"])
