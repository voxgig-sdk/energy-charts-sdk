...
func TestRenShareModelEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.RenShareModel(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.RenShareModel(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ENERGY_CHARTS_TEST_REN_SHARE_MODEL_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		renShareModelRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.ren_share_model")))
...
func ren_share_modelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ENERGY_CHARTS_TEST_REN_SHARE_MODEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ENERGY_CHARTS_TEST_REN_SHARE_MODEL_ENTID"])
