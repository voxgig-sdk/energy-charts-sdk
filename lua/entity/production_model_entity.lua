...
local ProductionModelEntity = {}
ProductionModelEntity.__index = ProductionModelEntity


function ProductionModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProductionModelEntity)
  self._name = "production_model"
  self._client = client
...
function ProductionModelEntity:get_name()
  return self._name
end


function ProductionModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProductionModelEntity.new(self._client, opts)
end
...
function ProductionModelEntity:mark_deleted()
  self._deleted = true
end


function ProductionModelEntity:deleted()
  return true == self._deleted
end


function ProductionModelEntity:data_set(args)
  if args ~= nil then
...
function ProductionModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProductionModelEntity:match_set(args)
  if args ~= nil then
...
function ProductionModelEntity:match_get()
...
function ProductionModelEntity:stream(action, args, callopts)
...
function ProductionModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
