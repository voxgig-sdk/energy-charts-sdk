...
local ShareModelEntity = {}
ShareModelEntity.__index = ShareModelEntity


function ShareModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShareModelEntity)
  self._name = "share_model"
  self._client = client
...
function ShareModelEntity:get_name()
  return self._name
end


function ShareModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShareModelEntity.new(self._client, opts)
end
...
function ShareModelEntity:mark_deleted()
  self._deleted = true
end


function ShareModelEntity:deleted()
  return true == self._deleted
end


function ShareModelEntity:data_set(args)
  if args ~= nil then
...
function ShareModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShareModelEntity:match_set(args)
  if args ~= nil then
...
function ShareModelEntity:match_get()
...
function ShareModelEntity:stream(action, args, callopts)
...
function ShareModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
