...
local InstalledModelEntity = {}
InstalledModelEntity.__index = InstalledModelEntity


function InstalledModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InstalledModelEntity)
  self._name = "installed_model"
  self._client = client
...
function InstalledModelEntity:get_name()
  return self._name
end


function InstalledModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InstalledModelEntity.new(self._client, opts)
end
...
function InstalledModelEntity:mark_deleted()
  self._deleted = true
end


function InstalledModelEntity:deleted()
  return true == self._deleted
end


function InstalledModelEntity:data_set(args)
  if args ~= nil then
...
function InstalledModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InstalledModelEntity:match_set(args)
  if args ~= nil then
...
function InstalledModelEntity:match_get()
...
function InstalledModelEntity:stream(action, args, callopts)
...
function InstalledModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
