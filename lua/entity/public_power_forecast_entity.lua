-- EnergyCharts SDK PublicPowerForecast entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PublicPowerForecastEntity = {}
PublicPowerForecastEntity.__index = PublicPowerForecastEntity


function PublicPowerForecastEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicPowerForecastEntity)
  self._name = "public_power_forecast"
  self._client = client
...
function PublicPowerForecastEntity:get_name()
  return self._name
end


function PublicPowerForecastEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicPowerForecastEntity.new(self._client, opts)
end
...
function PublicPowerForecastEntity:mark_deleted()
  self._deleted = true
end


function PublicPowerForecastEntity:deleted()
  return true == self._deleted
end


function PublicPowerForecastEntity:data_set(args)
  if args ~= nil then
...
function PublicPowerForecastEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicPowerForecastEntity:match_set(args)
  if args ~= nil then
...
function PublicPowerForecastEntity:match_get()
...
function PublicPowerForecastEntity:stream(action, args, callopts)
...
---@param reqmatch PublicPowerForecastListMatch
---@param ctrl? table
---@return PublicPowerForecast[]
---@return string? err
function PublicPowerForecastEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PublicPowerForecastEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PublicPowerForecastEntity
