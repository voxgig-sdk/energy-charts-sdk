...
local DailyAvgDictEntity = {}
DailyAvgDictEntity.__index = DailyAvgDictEntity


function DailyAvgDictEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DailyAvgDictEntity)
  self._name = "daily_avg_dict"
  self._client = client
...
function DailyAvgDictEntity:get_name()
  return self._name
end


function DailyAvgDictEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DailyAvgDictEntity.new(self._client, opts)
end
...
function DailyAvgDictEntity:mark_deleted()
  self._deleted = true
end


function DailyAvgDictEntity:deleted()
  return true == self._deleted
end


function DailyAvgDictEntity:data_set(args)
  if args ~= nil then
...
function DailyAvgDictEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DailyAvgDictEntity:match_set(args)
  if args ~= nil then
...
function DailyAvgDictEntity:match_get()
...
function DailyAvgDictEntity:stream(action, args, callopts)
...
function DailyAvgDictEntity:_run_op(ctx, post_done)
  local utility = self._utility
