-- EnergyCharts SDK RenShareModel entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RenShareModelEntity = {}
RenShareModelEntity.__index = RenShareModelEntity


function RenShareModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RenShareModelEntity)
  self._name = "ren_share_model"
  self._client = client
...
function RenShareModelEntity:get_name()
  return self._name
end


function RenShareModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RenShareModelEntity.new(self._client, opts)
end
...
function RenShareModelEntity:mark_deleted()
  self._deleted = true
end


function RenShareModelEntity:deleted()
  return true == self._deleted
end


function RenShareModelEntity:data_set(args)
  if args ~= nil then
...
function RenShareModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RenShareModelEntity:match_set(args)
  if args ~= nil then
...
function RenShareModelEntity:match_get()
...
function RenShareModelEntity:stream(action, args, callopts)
...
function RenShareModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
