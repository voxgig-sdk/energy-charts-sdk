-- EnergyCharts SDK Frequency entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FrequencyEntity = {}
FrequencyEntity.__index = FrequencyEntity


function FrequencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FrequencyEntity)
...
function FrequencyEntity:get_name()
  return self._name
end


function FrequencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FrequencyEntity.new(self._client, opts)
end
...
function FrequencyEntity:mark_deleted()
  self._deleted = true
end


function FrequencyEntity:deleted()
  return true == self._deleted
end


function FrequencyEntity:data_set(args)
  if args ~= nil then
...
function FrequencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FrequencyEntity:match_set(args)
  if args ~= nil then
...
function FrequencyEntity:match_get()
...
function FrequencyEntity:stream(action, args, callopts)
...
function FrequencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
