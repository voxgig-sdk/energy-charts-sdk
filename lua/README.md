...
The Lua SDK for the EnergyCharts API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CrossBorderModel(nil):load({ id = "example_id" }, nil)
...
local result, err = client:EnergyCharts(nil):load(
  { id = "test01" }, nil
)
