...
describe("PublicPowerForecastEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:PublicPowerForecast(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:PublicPowerForecast(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ENERGY_CHARTS_TEST_PUBLIC_POWER_FORECAST_ENTID JSON to run live")
      return
    end
...
function public_power_forecast_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ENERGY_CHARTS_TEST_PUBLIC_POWER_FORECAST_ENTID")
