...
class TestRenShareModelDirect:

    def test_should_direct_list_ren_share_model(self):
        setup = _ren_share_model_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ren_share_model", "live" if setup["live"] else "unit")
        if _skip:
...
def _ren_share_model_direct_setup(mockres):
