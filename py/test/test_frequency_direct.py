...
class TestFrequencyDirect:

    def test_should_direct_list_frequency(self):
        setup = _frequency_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-frequency", "live" if setup["live"] else "unit")
        if _skip:
...
def _frequency_direct_setup(mockres):
