...
class TestDailyAvgDictDirect:

    def test_should_direct_list_daily_avg_dict(self):
        setup = _daily_avg_dict_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-daily_avg_dict", "live" if setup["live"] else "unit")
        if _skip:
...
def _daily_avg_dict_direct_setup(mockres):
