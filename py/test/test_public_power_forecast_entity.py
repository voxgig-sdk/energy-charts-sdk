...
class TestPublicPowerForecastEntity:

    def test_should_create_instance(self):
        testsdk = EnergyChartsSDK.test(None, None)
        ent = testsdk.PublicPowerForecast(None)
    ...
    def test_should_stream(self):
        ...
        base = EnergyChartsSDK.test(seed, None)
        seen = list(base.PublicPowerForecast(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = EnergyChartsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.PublicPowerForecast(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _public_power_forecast_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "public_power_forecast." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ENERGY_CHARTS_TEST_PUBLIC_POWER_FORECAST_ENTID JSON to run live")
        client = setup["client"]
        ...
        public_power_forecast_ref01_data = None
        if len(public_power_forecast_ref01_data_raw) > 0:
...
def _public_power_forecast_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/public_power_forecast/PublicPowerForecastTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ENERGY_CHARTS_TEST_PUBLIC_POWER_FORECAST_ENTID"))
