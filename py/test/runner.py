...
class EnergyChartsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EnergyChartsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EnergyChartsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EnergyChartsTestRunner.getenv("ENERGY_CHARTS_TEST_LIVE")
        override = EnergyChartsTestRunner.getenv("ENERGY_CHARTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EnergyChartsTestRunner.getenv(key)
        ...
        explain = EnergyChartsTestRunner.getenv("ENERGY_CHARTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ENERGY_CHARTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EnergyChartsTestRunner._test_control is not None:
            return EnergyChartsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EnergyChartsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EnergyChartsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EnergyChartsTestRunner.load_test_control()
...
def load_env_local():
    EnergyChartsTestRunner.load_env_local()


def env_override(m):
    return EnergyChartsTestRunner.env_override(m)


def entity_data(v):
    return EnergyChartsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EnergyChartsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EnergyChartsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EnergyChartsTestRunner.load_test_control()


def live_client_options():
    return EnergyChartsTestRunner.live_client_options()


def live_delay_ms():
    return EnergyChartsTestRunner.live_delay_ms()
