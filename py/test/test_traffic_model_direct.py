...
class TestTrafficModelDirect:

    def test_should_direct_list_traffic_model(self):
        setup = _traffic_model_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-traffic_model", "live" if setup["live"] else "unit")
        if _skip:
...
def _traffic_model_direct_setup(mockres):
