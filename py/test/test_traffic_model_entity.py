...
class TestTrafficModelEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.TrafficModel(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.TrafficModel(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ENERGY_CHARTS_TEST_TRAFFIC_MODEL_ENTID JSON to run live")
        client = setup["client"]
        ...
        traffic_model_ref01_list_result = traffic_model_ref01_ent.list(traffic_model_ref01_match, None)
...
def _traffic_model_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ENERGY_CHARTS_TEST_TRAFFIC_MODEL_ENTID"))
