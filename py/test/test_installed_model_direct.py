...
class TestInstalledModelDirect:

    def test_should_direct_list_installed_model(self):
        setup = _installed_model_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-installed_model", "live" if setup["live"] else "unit")
        if _skip:
...
def _installed_model_direct_setup(mockres):
